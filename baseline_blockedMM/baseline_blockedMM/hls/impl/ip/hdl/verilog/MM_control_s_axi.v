...
// ==============================================================
`timescale 1ns/1ps
(* DowngradeIPIdentifiedWarnings="yes" *) module MM_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_C_DATA_1   = 7'h2c,
    ADDR_C_CTRL     = 7'h30,
    ADDR_ABC_DATA_0 = 7'h34,
    ADDR_ABC_DATA_1 = 7'h38,
    ADDR_ABC_CTRL   = 7'h3c,
    ADDR_N_DATA_0   = 7'h40,
    ADDR_N_CTRL     = 7'h44,
    ADDR_M_DATA_0   = 7'h48,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_ABC_DATA_0: begin
                    rdata <= int_ABC[31:0];
                end
                ADDR_ABC_DATA_1: begin
                    rdata <= int_ABC[63:32];
                end
                ADDR_N_DATA_0: begin
                    rdata <= int_N[31:0];
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ABC_DATA_0)
            int_ABC[31:0] <= (WDATA[31:0] & wmask) | (int_ABC[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ABC_DATA_1)
            int_ABC[63:32] <= (WDATA[31:0] & wmask) | (int_ABC[63:32] & ~wmask);
    end
end
